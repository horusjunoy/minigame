    ...
    public static class SyncObjectProcessor
    {
        ...
        public static List<FieldDefinition> FindSyncObjectsFields(Writers writers, Readers readers, Logger Log, TypeDefinition td, ref bool WeavingFailed)
        {
            ...
            foreach (FieldDefinition fd in td.Fields)
            {
                ...
                if (fd.FieldType.Resolve().IsDerivedFrom(WeaverRuntimeNames.SyncObject))
                {
                    if (fd.IsStatic)
                    {
                    ...
                    if (!fd.IsInitOnly)
                    {
                        ...
                        Log.Warning($"{fd.Name} should have a 'readonly' keyword in front of the variable because {WeaverRuntimeNames.SyncObject}s always need to be initialized by the Weaver.", fd);
