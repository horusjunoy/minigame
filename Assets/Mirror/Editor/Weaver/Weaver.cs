...
namespace Mirror.Weaver
{
    ...
    internal class Weaver
    {
        ...
        bool WeaveNetworkBehavior(TypeDefinition td)
        {
            ...
            if (!td.IsDerivedFrom(WeaverRuntimeNames.NetworkBehaviour))
            {
            ...
            while (parent != null)
            {
                if (parent.Is(WeaverRuntimeNames.NetworkBehaviour))
                {
                    break;
                }
        ...
        public bool Weave(AssemblyDefinition assembly, IAssemblyResolver resolver, out bool modified)
        {
            ...
            try
            {
                ...
                bool hasMirrorReference = CurrentAssembly.Name.Name == MirrorAssemblyName ||
                                          CurrentAssembly.MainModule.FindReference(MirrorAssemblyName) != null;
                if (!hasMirrorReference)
                {
                    // avoid resolving Mirror types for assemblies that don't reference Mirror
                    return true;
                }

                weaverTypes = new WeaverTypes(CurrentAssembly, resolver, Log, ref WeavingFailed);
                ...
                if (modified)
                {
                    ...
                    ReaderWriterProcessor.InitializeReaderAndWriters(CurrentAssembly, weaverTypes, writers, readers, GeneratedCodeClass, ref WeavingFailed);
