...
namespace Mirror.Weaver
{
    ...
    {
        public static TypeReference ResolveType(AssemblyDefinition assembly, IAssemblyResolver resolver, Logger Log, string fullName, ref bool WeavingFailed)
        {
            // try current module first
            TypeDefinition typeDef = assembly.MainModule.GetType(fullName);
            if (typeDef != null)
            {
            ...
            foreach (AssemblyNameReference asmRef in assembly.MainModule.AssemblyReferences)
            {
                ...
                try
                {
                    resolved = resolver.Resolve(asmRef);
                }
                ...
                TypeDefinition resolvedType = resolved.MainModule.GetType(fullName);
                if (resolvedType != null)
                {
                    return assembly.MainModule.ImportReference(resolvedType);
                }
            }

            Log.Error($"Failed to resolve type {fullName}");
            WeavingFailed = true;
            return null;
        }
