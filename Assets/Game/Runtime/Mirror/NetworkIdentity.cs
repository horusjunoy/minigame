    ...
    public sealed partial class NetworkIdentity : MonoBehaviour
    {
        ...
        void AssignAssetID(string path)
        {
            ...
            if (!string.IsNullOrWhiteSpace(path))
            {
                ...
                EditorBridge.RecordObject(this, "Assigned AssetId");

                // uint before = _assetId;
                string guidString = EditorBridge.AssetPathToGUID(path);
        ...
        {
            string path = EditorBridge.GetAssetPath(prefab);
            AssignAssetID(path);
        }
        ...
        void AssignSceneID()
        {
            ...
            if (sceneId == 0 || duplicate)
            {
                ...
                //    know how to fix it!
                if (EditorBridge.IsBuildingPlayer())
                ...
                // -> Undo.RecordObject is the new EditorUtility.SetDirty!
                // -> we need to call it before changing.
                EditorBridge.RecordObject(this, "Generated SceneId");
        ...
        void SetupIDs()
        {
            ...
            else if (EditorBridge.GetCurrentPrefabStage() != null)
            {
                ...
                if (EditorBridge.GetPrefabStage(gameObject) != null)
                {
                    // force 0 for prefabs
                    sceneId = 0;
                    ...
                    string path = EditorBridge.GetPrefabStageAssetPath(gameObject);
                    AssignAssetID(path);
                }
            }
        ...
        {
            static readonly Type UndoType = Type.GetType("UnityEditor.Undo, UnityEditor");
            static readonly MethodInfo UndoRecordObjectMethod = UndoType?.GetMethod(
                "RecordObject",
            ...
            static readonly Type AssetDatabaseType = Type.GetType("UnityEditor.AssetDatabase, UnityEditor");
            static readonly MethodInfo AssetPathToGUIDMethod = AssetDatabaseType?.GetMethod(
                "AssetPathToGUID",
                BindingFlags.Public | BindingFlags.Static,
                null,
                new[] { typeof(string) },
                null);
            static readonly MethodInfo GetAssetPathMethod = AssetDatabaseType?.GetMethod(
                "GetAssetPath",
                BindingFlags.Public | BindingFlags.Static,
                null,
            ...
            static readonly Type EditorApplicationType = Type.GetType("UnityEditor.EditorApplication, UnityEditor");
            static readonly PropertyInfo IsPlayingProperty = EditorApplicationType?.GetProperty(
                "isPlaying",
                BindingFlags.Public | BindingFlags.Static);

            static readonly Type BuildPipelineType = Type.GetType("UnityEditor.BuildPipeline, UnityEditor");
            static readonly PropertyInfo IsBuildingPlayerProperty = BuildPipelineType?.GetProperty(
                "isBuildingPlayer",
                BindingFlags.Public | BindingFlags.Static);

            static readonly Type PrefabStageUtilityType =
            ...
            static readonly MethodInfo GetCurrentPrefabStageMethod = PrefabStageUtilityType?.GetMethod(
                "GetCurrentPrefabStage",
                BindingFlags.Public | BindingFlags.Static);
            static readonly MethodInfo GetPrefabStageMethod = PrefabStageUtilityType?.GetMethod(
                "GetPrefabStage",
                BindingFlags.Public | BindingFlags.Static,
                null,
            ...
            {
                if (UndoType == null || UndoRecordObjectMethod == null)
                    return;

                UndoRecordObjectMethod.Invoke(null, new object[] { target, name });
            }

            public static string AssetPathToGUID(string path)
            {
                if (AssetDatabaseType == null || AssetPathToGUIDMethod == null)
                    return null;

                return AssetPathToGUIDMethod.Invoke(null, new object[] { path }) as string;
            }

            public static string GetAssetPath(UnityEngine.Object asset)
            {
                if (AssetDatabaseType == null || GetAssetPathMethod == null)
                    return null;

                return GetAssetPathMethod.Invoke(null, new object[] { asset }) as string;
            }

            public static bool IsPlaying()
            {
                if (EditorApplicationType == null || IsPlayingProperty == null)
                    return false;

                object value = IsPlayingProperty.GetValue(null);
                return value is bool isPlaying && isPlaying;
            }

            public static bool IsBuildingPlayer()
            {
                if (BuildPipelineType == null || IsBuildingPlayerProperty == null)
                    return false;

                object value = IsBuildingPlayerProperty.GetValue(null);
                return value is bool isBuildingPlayer && isBuildingPlayer;
            }

            public static object GetCurrentPrefabStage()
            {
                if (PrefabStageUtilityType == null || GetCurrentPrefabStageMethod == null)
                    return null;

                return GetCurrentPrefabStageMethod.Invoke(null, null);
            }

            public static object GetPrefabStage(GameObject gameObject)
            {
                if (PrefabStageUtilityType == null || GetPrefabStageMethod == null)
                    return null;

                return GetPrefabStageMethod.Invoke(null, new object[] { gameObject });
            }

            public static string GetPrefabStageAssetPath(GameObject gameObject)
            {
                object prefabStage = GetPrefabStage(gameObject);
                if (prefabStage == null)
                    return null;

                PropertyInfo assetPathProperty = prefabStage.GetType().GetProperty("assetPath", BindingFlags.Public | BindingFlags.Instance);
                if (assetPathProperty != null)
                {
                    return assetPathProperty.GetValue(prefabStage) as string;
                }

                PropertyInfo prefabAssetPathProperty = prefabStage.GetType().GetProperty("prefabAssetPath", BindingFlags.Public | BindingFlags.Instance);
                if (prefabAssetPathProperty != null)
                {
                    return prefabAssetPathProperty.GetValue(prefabStage) as string;
                }
