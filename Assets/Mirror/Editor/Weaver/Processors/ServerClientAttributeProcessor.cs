    ...
    static class ServerClientAttributeProcessor
    {
        ...
        static bool ProcessSiteMethod(WeaverTypes weaverTypes, Logger Log, MethodDefinition md, ref bool WeavingFailed)
        {
            if (md.Name == ".cctor" ||
                ...
                md.Name.StartsWith(WeaverRuntimeNames.InvokeRpcPrefix))
                return false;
        ...
        {
            if (md.HasCustomAttribute(WeaverRuntimeNames.ServerAttribute))
                InjectServerGuard(weaverTypes, md, true);
            else if (md.HasCustomAttribute(WeaverRuntimeNames.ServerCallbackAttribute))
                InjectServerGuard(weaverTypes, md, false);
            else if (md.HasCustomAttribute(WeaverRuntimeNames.ClientAttribute))
                InjectClientGuard(weaverTypes, md, true);
            else if (md.HasCustomAttribute(WeaverRuntimeNames.ClientCallbackAttribute))
