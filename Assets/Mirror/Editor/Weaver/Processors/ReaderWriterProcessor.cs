...
namespace Mirror.Weaver
{
    public static class ReaderWriterProcessor
    {
        ...
        static void ProcessMirrorAssemblyClasses(AssemblyDefinition CurrentAssembly, IAssemblyResolver resolver, Logger Log, Writers writers, Readers readers, ref bool WeavingFailed)
        {
            ...
            else if (CurrentAssembly.Name.Name != Weaver.MirrorAssemblyName)
            {
        ...
        {
            bool modified = false;
            if (!klass.IsAbstract && !klass.IsInterface && klass.ImplementsInterface(WeaverRuntimeNames.NetworkMessage))
            {
        ...
        static bool LoadDeclaredWriters(AssemblyDefinition currentAssembly, TypeDefinition klass, Writers writers)
        {
            ...
            foreach (MethodDefinition method in klass.Methods)
            {
                ...
                if (!method.Parameters[0].ParameterType.Is(WeaverRuntimeNames.NetworkWriter))
                    continue;
        ...
        static bool LoadDeclaredReaders(AssemblyDefinition currentAssembly, TypeDefinition klass, Readers readers)
        {
            ...
            foreach (MethodDefinition method in klass.Methods)
            {
                ...
                if (!method.Parameters[0].ParameterType.Is(WeaverRuntimeNames.NetworkReader))
                    continue;
        ...
        public static void InitializeReaderAndWriters(AssemblyDefinition currentAssembly, WeaverTypes weaverTypes, Writers writers, Readers readers, TypeDefinition GeneratedCodeClass, ref bool WeavingFailed)
        {
            ...
            writers.InitializeWriters(worker, ref WeavingFailed);
            readers.InitializeReaders(worker, ref WeavingFailed);
